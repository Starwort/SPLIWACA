import re
VALID_INTERPRETER_IDENTIFIERS = ['python','python3','python-official1']
...
def system_identifier(name: str) -> type:
    ...
    if name in ['int','INT','Int','integer','INTEGER','Integer']:
        return int
    if name in ['float','FLOAT','Float','real','REAL','Real','number','NUMBER','Number']:
        return float
    if name in [method(i) for method in (str.upper,str.lower,str.title) for i in ('list','array','tuple')]:
        return list
...
def handle_input(type_name:str, var_name:str, env:dict) -> None:
    ...
    if input_type in (str, int, float):
        ...
        while rv is None:
            try:
                rv = {'y':True,'yes':True,'n':False,'no':False}[input(INPUT_PROMPTS[input_type]).lower()]
            except Exception:
                print(f'That wasn\'t {TYPE_NAMES[input_type]}, try again')
        env[var_name] = rv
        ...
        while rv is None:
            try:
                ...
                match = re.fullmatch(r'((\+|\-?)\d+(\.\d+)?)??((\+|\-?)\d+(\.\d+)?i)?', tmp)
                real = int(match.group(1) or 0)
                imag = int((match.group(3) or '0i')[:-1])
